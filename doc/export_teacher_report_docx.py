...
SRC_MD = ROOT / "doc" / "给老师汇报稿-10分钟.md"
OUT_DIR = ROOT / "doc" / "generated"
OUT_MD = OUT_DIR / "老师汇报文稿_时间实验整理版.md"
OUT_DOCX = OUT_DIR / "老师汇报文稿_时间实验整理版.docx"
...
def save_doc_with_fallback(doc: Document, path: Path) -> Path:
    try:
        ...
        fallback = path.with_name(path.stem + "_更新版" + path.suffix)
        try:
            doc.save(fallback)
            return fallback
        except PermissionError:
            stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            fallback2 = path.with_name(path.stem + f"_{stamp}" + path.suffix)
            doc.save(fallback2)
            return fallback2
