...
layout(std430, binding = 4) readonly buffer PointInCellOffsets { int pointCellOff[]; };
layout(std430, binding = 5) readonly buffer PointInCellNeighbors { int pointCellNbr[]; };
layout(std430, binding = 6) readonly buffer PointValues { float pointVal[]; };
layout(std430, binding = 7) writeonly buffer Gradients { float grad[]; };
...
void hexaInterpolationFunctions(vec3 pcoords, out float w[8])
{
    float rm = 1.0 - pcoords.x;
    float sm = 1.0 - pcoords.y;
    float tm = 1.0 - pcoords.z;

    float rmXsm = rm * sm;
    float p0Xsm = pcoords.x * sm;
    float p0Xp1 = pcoords.x * pcoords.y;
    float rmXp1 = rm * pcoords.y;
...
bool vertexParametricCoords(int type, int localId, out vec3 pcoords)
{
    if (type == VTK_LINE) {
        if (localId == 0) { pcoords = vec3(0.0, 0.0, 0.0); return true; }
        if (localId == 1) { pcoords = vec3(1.0, 0.0, 0.0); return true; }
        return false;
    }
    ...
    if (type == VTK_TRIANGLE) {
        if (localId == 0) { pcoords = vec3(0.0, 0.0, 0.0); return true; }
        if (localId == 1) { pcoords = vec3(1.0, 0.0, 0.0); return true; }
        if (localId == 2) { pcoords = vec3(0.0, 1.0, 0.0); return true; }
        return false;
    }

    if (type == VTK_QUAD || type == VTK_HEXAHEDRON) {
        ...
        int nodeCount = (type == VTK_QUAD) ? 4 : 8;
        if (localId < 0 || localId >= nodeCount) {
            return false;
        }
        pcoords = coords[localId];
        return true;
    }

    if (type == VTK_PIXEL || type == VTK_VOXEL) {
        const vec3 coords[8] = vec3[8](
        ...
        int nodeCount = (type == VTK_PIXEL) ? 4 : 8;
        if (localId < 0 || localId >= nodeCount) {
            return false;
        }
        pcoords = coords[localId];
    ...
    if (type == VTK_TETRA) {
        ...
        if (localId == 1) { pcoords = vec3(1.0, 0.0, 0.0); return true; }
        if (localId == 2) { pcoords = vec3(0.0, 1.0, 0.0); return true; }
        if (localId == 3) { pcoords = vec3(0.0, 0.0, 1.0); return true; }
        return false;
    }

    if (type == VTK_WEDGE) {
        const vec3 coords[6] = vec3[6](
...
bool invertJacobian3DWithTolerance(vec3 tangents[3], float detTolerance, out float invJ[9])
{
    float j00 = tangents[0].x;
    float j01 = tangents[0].y;
    float j02 = tangents[0].z;
...
bool pointParametricCoordsForCell(int cellId, int type, int pointId, int localId, out vec3 pcoords)
{
    if (type != VTK_HEXAHEDRON) {
        return vertexParametricCoords(type, localId, pcoords);
    }
    ...
    vec3 target = loadPoint(pointId);
    vec3 xOrigin = loadPoint(cellConn[begin]);
    vec3 targetRel = target - xOrigin;
    pcoords = vec3(0.5);

    const ivec2 diagonals[4] = ivec2[4](
        ivec2(0, 6), ivec2(1, 7), ivec2(2, 4), ivec2(3, 5)
    );
    float longestDiag2 = 0.0;
    for (int i = 0; i < 4; ++i) {
        vec3 a = loadPoint(cellConn[begin + diagonals[i].x]);
        vec3 b = loadPoint(cellConn[begin + diagonals[i].y]);
        vec3 d = a - b;
        longestDiag2 = max(longestDiag2, dot(d, d));
    }
    float volumeBound = longestDiag2 * sqrt(max(longestDiag2, 0.0));
    float detTolerance = max(1e-20, 1e-5 * volumeBound);

    for (int iter = 0; iter < 10; ++iter) {
        float w[8];
        hexaInterpolationFunctions(pcoords, w);

        vec3 dNdXi[8];
        int derivDim = 0;
        int derivNodeCount = 0;
        if (!shapeFunctionDerivatives(type, pcoords, dNdXi, derivDim, derivNodeCount) ||
            derivDim != 3 || derivNodeCount != 8) {
            return false;
        }
        ...
        tangents[2] = vec3(0.0);
        for (int i = 0; i < 8; ++i) {
            vec3 x = loadPoint(cellConn[begin + i]) - xOrigin;
            mapped += x * w[i];
            tangents[0] += x * dNdXi[i].x;
            tangents[1] += x * dNdXi[i].y;
            tangents[2] += x * dNdXi[i].z;
        }

        float invJ[9];
        if (!invertJacobian3DWithTolerance(tangents, detTolerance, invJ)) {
            return false;
        }

        vec3 residual = mapped - targetRel;
        vec3 delta = vec3(
            residual.x * invJ[0] + residual.y * invJ[1] + residual.z * invJ[2],
            residual.x * invJ[3] + residual.y * invJ[4] + residual.z * invJ[5],
            residual.x * invJ[6] + residual.y * invJ[7] + residual.z * invJ[8]
        );
        vec3 next = pcoords - delta;

        vec3 step = abs(next - pcoords);
        pcoords = next;
        if (step.x < 1e-6 && step.y < 1e-6 && step.z < 1e-6) {
            return true;
        }
        if (abs(pcoords.x) > 1e6 || abs(pcoords.y) > 1e6 || abs(pcoords.z) > 1e6) {
            return false;
        }
    }
...
void main()
{
    ...
    int pointId = int(iu);
    int outBase = pointId * (3 * uNumComponents);
    ...
    for (int idx = begin; idx < end; ++idx) {
        ...
        int cellBegin = cellOff[cellId];
        int cellEnd = cellOff[cellId + 1];
        int localId = -1;
        for (int j = cellBegin; j < cellEnd; ++j) {
            if (cellConn[j] == pointId) {
                localId = j - cellBegin;
                break;
            }
        }
        ...
        vec3 pcoords;
        if (!pointParametricCoordsForCell(cellId, cellType[cellId], pointId, localId, pcoords)) {
            continue;
        }
    ...
    for (int comp = 0; comp < uNumComponents; ++comp) {
        grad[outBase + comp * 3 + 0] *= invUsed;
        grad[outBase + comp * 3 + 1] *= invUsed;
        grad[outBase + comp * 3 + 2] *= invUsed;
    }
}
